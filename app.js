...
${document.body.innerHTML}
    ...
    async showBrowsePage(cursor = null) {
        ...
        const browseDesc = isLoggedIn
            ? 'Posts from your feed. Make wiki articles from anything you find.'
            : 'Discover posts from the Bluesky feed. Make wiki articles from anything you find.';
        container.innerHTML = `
            <div class="browse-page-header">
                <h1>${browseTitle}</h1>
                <p class="browse-page-desc">${browseDesc}</p>
                <div class="browse-loading" id="browse-loading">Loading posts…</div>
            </div>
            <div id="browse-posts-list" class="browse-posts-list"></div>
        ...
        const listEl = document.getElementById('browse-posts-list');
        ...
        try {
            const { posts, cursor: nextCursor } = await this.storage.fetchBrowsePosts(cursor, 25);
            this.browseFeedCursor = nextCursor;
            this.browsePosts = posts;
            loadingEl.style.display = 'none';
            
            if (posts.length === 0) {
                listEl.innerHTML = '<p class="archive-empty">No posts in this batch. Try again later.</p>';
            } else {
                listEl.innerHTML = posts.map((post, idx) => {
                    const author = (post.authorHandle || '').replace(/"/g, '&quot;');
                    const displayName = (post.authorDisplayName || '').replace(/"/g, '&quot;');
                    const text = (post.text || '').replace(/</g, '&lt;').replace(/>/g, '&gt;');
                    const postUrlSafe = (post.postUrl || '').replace(/"/g, '&quot;');
                    const imagesHtml = (post.imageUrls || []).map((img, i) => {
                        const imgIdx = `${idx}-img-${i}`;
                        return `
                            <div class="browse-post-image-wrap">
                                <img src="${img.url}" alt="${(img.alt || '').replace(/"/g, '&quot;')}" class="browse-post-image" loading="lazy">
                                <button type="button" class="btn-secondary btn-small browse-add-img-btn" data-post-idx="${idx}" data-img-idx="${i}">Add to collection</button>
                            </div>`;
                    }).join('');
                    return `
                        <article class="browse-post-card" data-post-idx="${idx}">
                            <div class="browse-post-header">
                                <a href="${postUrlSafe}" target="_blank" rel="noopener" class="browse-post-author">${displayName ? this.escapeHtml(displayName) : ''} <span class="browse-post-handle">@${this.escapeHtml(author)}</span></a>
                            </div>
                            ${text ? `<div class="browse-post-text">${this.escapeHtml(text).replace(/\n/g, '<br>')}</div>` : ''}
                            ${imagesHtml ? `<div class="browse-post-images">${imagesHtml}</div>` : ''}
                            <div class="browse-post-actions">
                                <button type="button" class="btn-primary btn-small browse-wiki-btn" data-post-idx="${idx}">Make wiki article</button>
                                <a href="${postUrlSafe}" target="_blank" rel="noopener" class="btn-secondary btn-small">View on Bluesky</a>
                            </div>
                        </article>`;
                }).join('');
                
                listEl.querySelectorAll('.browse-wiki-btn').forEach(btn => {
                    const idx = parseInt(btn.getAttribute('data-post-idx'), 10);
                    btn.addEventListener('click', () => this.createArticleFromAtProtocolPost(this.browsePosts[idx]));
                });
                listEl.querySelectorAll('.browse-add-img-btn').forEach(btn => {
                    const postIdx = parseInt(btn.getAttribute('data-post-idx'), 10);
                    const imgIdx = parseInt(btn.getAttribute('data-img-idx'), 10);
                    const post = this.browsePosts[postIdx];
                    const img = post?.imageUrls?.[imgIdx];
                    if (post && img) {
                        btn.addEventListener('click', () => this.addBrowseImageToCollection(post, img));
                    }
                });
            }
    ...
    createArticleFromAtProtocolPost(post) {
        const title = (post.text || 'Post from AT Protocol').slice(0, 50).trim() + (post.text && post.text.length > 50 ? '…' : '');
        const slug = title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'atproto-post-' + Date.now();
        const sourceLine = post.postUrl ? '\n\nSource: ' + post.postUrl : '';
        const content = (post.text || '').trim() + sourceLine;
        document.getElementById('modal-title').textContent = 'Make wiki article';
        document.getElementById('article-title').value = title;
        document.getElementById('article-key').value = slug;
        document.getElementById('article-key').dataset.manual = '';
        document.getElementById('article-key').disabled = false;
        document.getElementById('article-key-group').style.display = 'none';
        if (this.quill) {
            ...
            const fallback = document.getElementById('article-content-fallback');
        ...
        this.currentArticleKey = null;
        this.hideCreateButton();
        this.switchCreateTab('article');
        this.updateAlbumSelect();
        setTimeout(() => document.getElementById('article-title').focus(), 100);
    }

    async addBrowseImageToCollection(post, img) {
        const archiveItem = {
            imageUrl: img.url,
            name: post.authorHandle ? `@${post.authorHandle}` : 'From feed',
            type: 'image',
            source: post.postUrl || '',
            albumIds: [],
            assignmentType: 'albums',
        ...
        try {
            ...
            this.showUpdateNotification('Failed to add: ' + (e.message || 'Unknown error'));
        }
    }
