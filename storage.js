...
class WikiStorage {
    ...
    // Normalize feed item to a post view for browsing (full post with text + embed).
    _normalizeFeedItemToPost(item) {
        const post = item?.post;
        if (!post) return null;
        const author = post.author || {};
        const handle = author.handle || 'unknown';
        ...
        const postUrl = `https://bsky.app/profile/${encodeURIComponent(handle)}/post/${encodeURIComponent(rkey)}`;
        ...
        if (embed?.images && Array.isArray(embed.images)) {
            for (const img of embed.images) {
                ...
                const cid = ref?.$link || ref;
                if (cid) {
                    const url = this.getAtProtocolBlobUrl(cid, did);
                    if (url) imageUrls.push({ url, alt: img.alt || '' });
                }
            }
        }
        if (embed?.media?.images && Array.isArray(embed.media.images)) {
            for (const img of embed.media.images) {
                const ref = img?.ref || img;
                const cid = ref?.$link || ref;
                if (cid) {
                    const url = this.getAtProtocolBlobUrl(cid, did);
                    if (url) imageUrls.push({ url, alt: '' });
                }
            }
        }
        return {
    ...
    // Fetch AT Protocol feed as full posts (for browse + Make wiki article).
    async fetchBrowsePosts(cursor = null, limit = 25) {
        ...
        const posts = feed.map((item) => this._normalizeFeedItemToPost(item)).filter(Boolean);
